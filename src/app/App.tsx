...
import { Plus, Trash2, Copy, Save, Pencil, X, Search, ChevronsUpDown, ChevronRight, ChevronDown } from 'lucide-react'
...
export default function App() {
  ...
  function deleteWO(customerId: string, projectId: string, woId: string) {
    setDb(prev => prev.map(c => (c.id !== customerId ? c : { ...c, projects: c.projects.map(p => (p.id !== projectId ? p : { ...p, wos: p.wos.filter(w => w.id !== woId) })) })))
  }
  function deletePO(customerId: string, projectId: string, poId: string) {
    setDb(prev => prev.map(c => (c.id !== customerId ? c : { ...c, projects: c.projects.map(p => (p.id !== projectId ? p : { ...p, pos: p.pos.filter(po => po.id !== poId) })) })))
  }
  function addWO(customerId: string, projectId: string, data: { number: string; type: WOType; note?: string }) {
    ...
    setDb(prev => prev.map(c => (c.id !== customerId ? c : { ...c, projects: c.projects.map(p => (p.id !== projectId ? p : { ...p, wos: [...p.wos, { id: uid('wo'), ...data }] })) })))
  }
  function addPO(customerId: string, projectId: string, data: { number: string; note?: string }) {
    const uid = (p: string) => `${p}_${Math.random().toString(36).slice(2,9)}${Date.now().toString(36).slice(-4)}`
    setDb(prev => prev.map(c => (c.id !== customerId ? c : { ...c, projects: c.projects.map(p => (p.id !== projectId ? p : { ...p, pos: [...p.pos, { id: uid('po'), ...data }] })) })))
  }
  function addProject(customerId: string, projectNumber: string) {
    const uid = (p: string) => `${p}_${Math.random().toString(36).slice(2,9)}${Date.now().toString(36).slice(-4)}`
    setDb(prev => prev.map(c => (c.id !== customerId ? c : { ...c, projects: [...c.projects, { id: uid('proj'), number: projectNumber, wos: [], pos: [] }] })))
  }
  function createCustomer(data: Omit<Customer, 'id' | 'projects'>) {
    const uid = (p: string) => `${p}_${Math.random().toString(36).slice(2,9)}${Date.now().toString(36).slice(-4)}`
    const c: Customer = { id: uid('cust'), ...data, projects: [] }
    setDb(prev => [c, ...prev]); setSelectedCustomerId(c.id)
  }

  function EditableField({ label, value, onSave, fieldKey, placeholder }: { label: string; value?: string; onSave: (v: string) => void; fieldKey: string; placeholder?: string }) {
                  ...
                  <div className='mt-3 rounded-xl border border-zinc-700/40 p-3'>
                    ...
                    <div className='grid gap-2 md:grid-cols-5'>
                      <div className='md:col-span-2'>
                        ...
                        <Input value={woForm.number} onChange={(e) => setWoForm({ ...woForm, number: (e.target as HTMLInputElement).value })} placeholder='WO-12345' />
                      </div>
                      <div>
                        <Label>Type</Label>
                    ...
                    <div className='mt-2'>
                      <Button onClick={() => { if (!woForm.number.trim()) return; addWO(customer.id, project.id, { number: woForm.number.trim(), type: woForm.type, note: woForm.note?.trim() || undefined }); setWoForm({ number: '', type: 'Build', note: '' }) }}>
                        <Plus size={16} /> Add WO
                      </Button>
                    </div>
                  </div>
      ...
      <div className='mx-auto max-w-6xl'>
        ...
        <Card className='mb-6'>
          ...
          <CardContent>
            <div className='grid gap-3 md:grid-cols-3'>
              <div>
                <Label>Customer (type or pick)</Label>
                <div className='flex items-center gap-2'>
                  <Input value={customerQuery} onChange={(e) => setCustomerQuery((e.target as HTMLInputElement).value)} placeholder='Start typing a name…' />
                  <div className='relative'>
                    <details className='group'>
                      <summary className='flex cursor-pointer list-none items-center gap-1 rounded-xl border border-zinc-600/40 px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-800/40'>
                        <ChevronsUpDown size={16} />
                        Pick
                      </summary>
                      <div className='absolute z-10 mt-2 max-h-64 w-56 overflow-auto rounded-xl border border-zinc-700/50 bg-zinc-900 p-2 shadow-xl'>
                        {customerOptions.map((name) => (
                          <button key={name} onClick={() => setCustomerQuery(name)} className='block w-full rounded-lg px-2 py-1 text-left text-sm hover:bg-zinc-800'>
                            {name}
                          </button>
                        ))}
                      </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className='mt-3'>
              <Button size='lg' onClick={() => { if (!newCust.name.trim()) return; createCustomer({ name: newCust.name.trim(), address: newCust.address.trim() || undefined, contactName: newCust.contactName.trim() || undefined, contactPhone: newCust.contactPhone.trim() || undefined, contactEmail: newCust.contactEmail.trim() || undefined }); setNewCust({ name: '', address: '', contactName: '', contactPhone: '', contactEmail: '' }) }}>
  ...
  return (
    <div className='flex items-end gap-2'>
      <div className='flex-1'>
        <Label>Project Number</Label>
        <Input value={val} onChange={(e) => setVal((e.target as HTMLInputElement).value)} placeholder='e.g. PRJ-2025-123' />
      </div>
      <Button onClick={() => { if (!val.trim()) return; onAdd(val.trim()); setVal('') }}>
        <Plus size={16} /> Add
      </Button>
    </div>
  )
